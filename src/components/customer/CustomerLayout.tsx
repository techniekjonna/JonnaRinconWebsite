...
const CustomerLayout: React.FC<CustomerLayoutProps> = ({ children }) => {
  ...
  const isOnDashboard = location.pathname === '/customer/dashboard' || location.pathname === '/customer';
  ...
  const setSidebarPosition = (pos: SidebarPosition) => {
    localStorage.setItem('customer-sidebar-position', pos);
    setSidebarPositionState(pos);
  };

  const menuItems = [
    { label: 'DASHBOARD', subtitle: 'Overview & stats', href: '/customer/dashboard' },
    { label: 'SHOP BEATS', subtitle: 'Browse & purchase', href: '/customer/shop' },
    { label: 'MY ORDERS', subtitle: 'Order history', href: '/customer/orders' },
    { label: 'MY PRODUCTS', subtitle: 'Downloads & licenses', href: '/customer/my-products' },
    { label: 'COLLABORATIONS', subtitle: 'Active collabs', href: '/customer/collaborations' },
    { label: 'BECOME ARTIST', subtitle: 'Request artist role', href: '/customer/request-artist-role' },
    { label: 'CHAT', subtitle: 'Messages', href: '/customer/chat' },
    { label: 'PROFILE', subtitle: 'Your profile', href: '/customer/profile' },
    { label: 'SETTINGS', subtitle: 'Account settings', href: '/customer/settings' },
  ];
  ...
  const goBack = () => { try { navigate(-1); } catch { navigate('/customer/dashboard'); } };
        ...
        >
          ...
          <div className="px-5 py-4 flex-shrink-0">
            <div className="flex items-center gap-1.5 mb-4">
              <button onClick={() => navigate('/customer/dashboard')} className="p-2 rounded-lg text-white/40 hover:bg-white/[0.04] hover:text-white/80 transition" title="Dashboard"><LayoutDashboard size={16} /></button>
              <button onClick={() => navigate('/customer/settings')} className="p-2 rounded-lg text-white/40 hover:bg-white/[0.04] hover:text-white/80 transition" title="Settings"><Settings size={16} /></button>
              <button onClick={cycleSidebarPosition} className="p-2 rounded-lg bg-purple-600/20 border border-purple-600/30 text-purple-400 hover:bg-purple-600/30 transition" title="Sidebar positie">{positionIcon}</button>
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
                {user?.displayName?.[0] || user?.email?.[0] || 'C'}
              </div>
              <div className="min-w-0">
                <p className="text-xs font-medium text-white truncate">{user?.displayName || 'Customer'}</p>
    ...
    <div className="min-h-screen bg-black">
      <div className="fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-6 md:px-10 py-4 md:py-5">
        <div className="flex-shrink-0">
          ...
          {isOnDashboard && <h1 className="text-lg font-bold text-white">Dashboard</h1>}
        </div>
            ...
            <div className="relative z-10 h-full flex flex-col px-8 md:px-12">
                  ...
                  >
                    <div className="flex items-center justify-between transition-transform duration-300 group-hover:translate-x-2">
                      <div>
                        ...
                        <span className="block text-xs text-white/25 mt-1 uppercase tracking-widest font-medium group-hover:text-purple-400/60 transition-colors duration-300">{item.subtitle}</span>
                      </div>
                      <ArrowUpRight className="w-5 h-5 text-white/10 group-hover:text-purple-400/50 transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                    </div>
                  </button>
                ))}
                <div className="flex flex-col gap-4 pt-6">
                  <div className="flex items-center justify-center gap-3">
                    <button onClick={() => { navigate('/customer/dashboard'); closeMenu(); }} className="p-2.5 rounded-xl transition-all duration-200 text-white/40 hover:bg-white/[0.04] hover:text-white/80" title="Dashboard"><LayoutDashboard size={18} /></button>
                    <button onClick={() => { navigate('/customer/settings'); closeMenu(); }} className="p-2.5 rounded-xl transition-all duration-200 text-white/40 hover:bg-white/[0.04] hover:text-white/80" title="Settings"><Settings size={18} /></button>
                    <button onClick={cycleSidebarPosition} className={`p-2.5 rounded-xl border transition-all duration-200 ${sidebarPosition !== 'floating' ? 'bg-purple-600/20 border-purple-600/40 text-purple-400 hover:bg-purple-600/30' : 'border-transparent text-white/40 hover:bg-white/[0.04] hover:text-white/80'}`} title="Sidebar positie">{positionIcon}</button>
                  ...
                  <button onClick={() => { closeMenu(); handleSignOut(); }} className="text-left cursor-pointer w-full">
                    <span className="text-sm uppercase tracking-widest text-white/20 hover:text-purple-400 transition-colors duration-300 font-medium">Sign Out</span>
                  </button>
                </div>
              </div>
              <div className="w-full h-px bg-white/[0.06]" />
              <div className="flex-shrink-0 py-6 md:py-8">
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-white truncate">{user?.displayName || 'Customer'}</p>
